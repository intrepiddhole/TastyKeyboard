    ...
    */
    ...
    override func loadView() {
        ...
        if let aBanner = self.createBanner() {
            ...
            let left = NSLayoutConstraint(item: self.scrollView!, attribute: NSLayoutAttribute.Left, relatedBy: NSLayoutRelation.Equal, toItem: self.bannerView!, attribute: NSLayoutAttribute.Left, multiplier: 1, constant: 5)
            let right = NSLayoutConstraint(item: self.scrollView!, attribute: NSLayoutAttribute.Right, relatedBy: NSLayoutRelation.Equal, toItem: self.bannerView!, attribute: NSLayoutAttribute.Right, multiplier: 1, constant: -5)
            let top = NSLayoutConstraint(item: self.scrollView!, attribute: NSLayoutAttribute.Top, relatedBy: NSLayoutRelation.Equal, toItem: self.bannerView!, attribute: NSLayoutAttribute.Top, multiplier: 1, constant: 5)
            let bottom = NSLayoutConstraint(item: self.scrollView!, attribute: NSLayoutAttribute.Bottom, relatedBy: NSLayoutRelation.Equal, toItem: self.bannerView!, attribute: NSLayoutAttribute.Bottom, multiplier: 1, constant: -5)
            
            self.bannerView?.addConstraint(left)
            self.bannerView?.addConstraint(right)
            self.bannerView?.addConstraint(top)
            self.bannerView?.addConstraint(bottom)
        ...
        let upSwipe = UISwipeGestureRecognizer(target: self, action: Selector("openSuggestion:"))
        upSwipe.direction = .Up
        self.view!.addGestureRecognizer(upSwipe)
        
        let downSwipe = UISwipeGestureRecognizer(target: self, action: Selector("closeSuggestion:"))
        downSwipe.direction = .Down
        self.view!.addGestureRecognizer(downSwipe)
    }
    
    func openSuggestion(sender:UISwipeGestureRecognizer) {
        self.bannerView?.hidden = false
        self.keyboardHeight = self.heightForOrientation(self.interfaceOrientation, withTopBanner: true)
    }
    
    func closeSuggestion(sender:UISwipeGestureRecognizer) {
    ...
    */
    ...
    func keyPressed(key: Key) {
        ...
        if let proxy = (self.textDocumentProxy as? UIKeyInput) {
            ...
            var s = self.textDocumentProxy.documentContextBeforeInput
            s?.stringByReplacingOccurrencesOfString(" ", withString: "%20")
            let urlPath: String = "http://178.62.216.52:8080/v1/test"
            var url: NSURL = NSURL(string: urlPath)!
            var request: NSURLRequest = NSURLRequest(URL: url)
            let queue:NSOperationQueue = NSOperationQueue()
            NSURLConnection.sendAsynchronousRequest(request, queue: queue, completionHandler:{ (response: NSURLResponse?, data: NSData?, error: NSError?) -> Void in
                var dataString = String(data: data!, encoding: NSUTF8StringEncoding)
                var index1 = dataString!.startIndex.advancedBy(2)
                dataString = dataString?.substringFromIndex(index1)
                var index2 = dataString!.endIndex.advancedBy(-2)
                dataString = dataString?.substringToIndex(index2)
                self.imgUrls = dataString?.componentsSeparatedByString("\",\"")
    ...
    func checkgifImage(imgData : NSData) -> Bool {
        var c : [UInt8] = [0]
    ...
    func refreshBannerView()
    {
        if(self.bannerView != nil){
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) {
                ...
                var totalcount = self.imgUrls?.count
                if(totalcount > 5){
                    totalcount = 5
                }
                
                for(var i = 0 ; i < totalcount ; i++){
                    var path = NSURL.init(string: self.imgUrls![i])
                    let imgdata = NSData(contentsOfURL: path!)
                    if(imgdata != nil){
                        if(self.checkgifImage(imgdata!)){
                            var image = UIImage.gifWithData(imgdata!)
                            if(image != nil){
                                if(xpos != 0){
                                    xpos += 5
                                }
                                var newImageview = UIImageView.init(image: image)
                                var imageWidth = rt!.size.height*image!.size.width/image!.size.height
                                newImageview.frame = CGRectMake(xpos, (CGFloat)(0), imageWidth, (CGFloat)(rt!.size.height))
                                //dispatch_async(dispatch_get_main_queue()) {
                                self.scrollView?.addSubview(newImageview)
                                //}
                                //UIPasteboard.generalPasteboard().image = image
                                xpos = xpos + imageWidth
                                self.scrollView?.contentSize = CGSizeMake(xpos, 1)
                            }
                        }else{
                            //var image = UIImage.jpgWithData(imgdata!)
                            var image = UIImage.init(data: imgdata!)
                            if(image != nil){
                                if(xpos != 0){
                                    xpos += 5
                                }
